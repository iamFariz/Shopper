...
public class MainActivity extends JFrame {
    ...
    public void setTopPanel(){
        ...
        _labelTop.setText("ShopMart. " +
                "The Best Shopping Apps....");
        Dimension _labelSize = _labelTop.getPreferredSize();

        _labelTop.setBounds(150,100,_labelSize.width,
                _labelSize.height);
        _labelTop.setFont(new Font("Serif",Font.PLAIN,24));
        _panelTop.setBounds(10,20, this._screenWidth - 20,
                38);
    ...
    private void setStokPanel() throws IOException {
        Border _borderMiddle = BorderFactory.createTitledBorder
                ("Select Products");
        JPanel _middlePanel = new JPanel(new FlowLayout(FlowLayout.LEFT));
        _middlePanel.setBounds(10,70,this._screenWidth - 280,
        ...
        for(int index=0;index<_listItem.size();index++){
            ...
            _borderProducts[index] = BorderFactory.createTitledBorder(
                    _listItem.get(index).get_pluName());
            ...
            _labelPic[index] = new JLabel("<html>Code : "+
                    _listItem.get(index).get_pluCode()+"<br/>Name : "+ _listItem.get(index).get_pluName()+"<br/>Size : "+
                    _listItem.get(index).get_size()+"<br/>"+
                    _listItem.get(index).get_description()+
                    "<br/>Price : "+_listItem.get(index).get_price()+"<br/>");
            String _PATH = "D:\\CodingS2\\Pictures\\";
            ...
            Image newimg1 = image1.getScaledInstance(50,
                    50,  Image.SCALE_SMOOTH); // scale it the smooth way
    ...
    public void AddtoCart(){
        ...
        for(int index=0;index<listCart.size();index++){
            modelList.addElement(listCart.get(index).getPluCode().toString()+" - " + listCart.get(index).getPluName()+" - "+listCart.get(index).getPluQty());
        }
